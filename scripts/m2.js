	...
	parse: function (data) {
		...
		var parser = new jParser(data, {
			...
			string0: function (length) {
				return this.parse(['string', length]).substr(0, length - 1);
			},
			...
			struct: function (type) {
				...
				return this.seek(nofs.offset, function () {
					if (type === 'char') {
						return that.parse(['string0', nofs.count]);
					} else {
						return that.parse(['array', type, nofs.count]);
					}
				});
			},
			...
			views: function () {
				var num = this.parse('uint32');
				var views = [];
				num = 1; // fix
				for (var i = 0; i < num; ++i) {
					that.pushRequest();
					var filename = that.filename.replace(/\.m2$/, '0' + i + '.skin');
					(function (i) {
						new M2(filename,
							function (skin) {
								views[i] = skin;
